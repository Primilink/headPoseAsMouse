...
while cap.isOpened():
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            if y < -HORIZONTAL_TOL:
                text = "Looking Left"
            elif y > HORIZONTAL_TOL:
                if(text != "Looking Right"):
                    #pyautogui.click(x=mouseX, y=mouseY, clicks=1, interval=0, button='left')
                    pyautogui.mouseDown() 
                text = "Looking Right"
            elif x < -VERTICAL_TOL:
                pyautogui.scroll(-SCROLL_VALUE, x=mouseX, y=mouseY)
                text = "Looking Down"
